...
## MacBook Liquid Damage — Liquid Contact Indicators, Short Circuit Failure Modes, and Component-Level Diagnostics

> **[Use This Script To Fix - Click Here](https://error-number-472173.github.io/.github/MacBook-liquid-damage)** — macOS 12 Monterey through macOS 26.5 Tahoe, Intel and Apple Silicon.

**What's actually responsible for MacBook Liquid Damage, in order of architectural impact:**
...
- Third-party kernel extensions (`kext`) or System Extensions interfering with the macOS subsystem responsible for MacBook Liquid Damage
...
## Internal macOS Architecture for MacBook Liquid Damage

macOS organizes all system functionality into discrete architectural layers. MacBook Liquid Damage involves the following components:

| Layer | Component | Relevance to MacBook Liquid Damage |
|---|---|---|
...
| APFS Volume | Contents | Relevance to MacBook Liquid Damage |
|---|---|---|
...
## SMC Architecture and Its Role in MacBook Liquid Damage
...
| SMC Domain | Components Managed | Failure Impact on MacBook Liquid Damage |
|---|---|---|
...
A corrupted or inconsistent TCC database produces hardware access failures that appear at the framework level — the hardware is fully functional, but the framework layer refuses to pass data to the requesting application. TCC corruption is a common cause of MacBook Liquid Damage appearing after a macOS update, because the update may invalidate stored bundle identifier checksums without regenerating the access grants.

---

## Diagnostic Data Sources

| Tool | Access | What It Reveals for MacBook Liquid Damage |
|---|---|---|
...
If MacBook Liquid Damage does not occur in Safe Boot, the cause is categorically in third-party software or accumulated user configuration loaded during normal boot — not in hardware or core macOS components.

---

## New User Account Isolation

Creating a temporary standard user account (System Settings → Users & Groups → Add Account) and reproducing the conditions that trigger MacBook Liquid Damage:
...
*This reference covers MacBook Liquid Damage as observed on macOS 10.13 High Sierra through macOS 26.5 Tahoe on Intel x86_64 and Apple Silicon ARM64 hardware.*
